...
function Intro({
  ...
  return (
    <Section className={cx('container')}>
      ...
      {/* dangerouslySetInnerHTML은 스트링으로 Html을 만든다 이따 &gt 나 /n 같은 것들이 그대로 표출안되고 파싱되어 표출시키도록 한다 */}
      {/* 최근엔 보안상 이슈로 잘 사용안한다. */}
      {/* 따라서 &lt은  <  이렇게 변경해서 넣어주면 된다. */}
      {/* &gt 는 단순히 > 이렇게 변경해서 넣어주면 된다. */}
      {/* 줄바꿈 /n은 무시한다. 따라서 텍스트 컴포넌트를 만든다. */}
